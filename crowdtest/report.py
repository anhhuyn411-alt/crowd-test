...
def build_markdown(crew: CrewResult, generated_at: str | None = None) -> str:
    ...
    lines = [
        ...
        "",
        f"*Generated {ts} by [crowd-test](https://github.com/anhhuyn411/crowd-test)*",
        "",
        "## Verdict",
        "",
...
<div class="wrap">
  ...
  {% if findings %}
    ...
    {% endfor %}
  {% else %}
    <p>No findings — the crowd had a smooth ride. 🎉</p>
  {% endif %}
  ...
  <div class="persona">
    {% if r.ok %}
      <span class="score {{ 'good' if (r.satisfaction_score or 0) >= 7 else ('bad' if (r.satisfaction_score or 0) <= 4 else '') }}">
        {{ r.satisfaction_score if r.satisfaction_score is not none else '–' }}/10</span>
    ...
    {% else %}
      <strong>{{ r.display_name }}</strong> — session failed
      <div class="error">{{ r.error }}</div>
    {% endif %}
  </div>
  {% endfor %}

  <footer>made with <a href="https://github.com/anhhuyn411/crowd-test">crowd-test</a>
    — hire a crowd of AI users to test your app</footer>
</div>
</body>
</html>
"""
